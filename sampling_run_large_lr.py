...
for lr in lrs:
    if args.load is None:
        log_file = args.train_file.split(".")[0] + "_" + "BERT-large" + "_" + str(args.percentage) + "_" + str(args.shots) + "_" + lr + ".txt"
        predict_cmd = \
            "python3 " + args.train_file + \
            " --learning-rate " +  str(lr) + \
            " --shots " +  str(args.shots) + \
            " --dataseed 9595 " + " --model-type bert-large-uncased " + " > " + log_file
    else:
...
# srun --gres=gpu:3  --qos general -t 1000 python sampling_run_large_lr.py   --load /home/woojeong2/vok_pretraining/snap/vlpretrain/bert_resnext_combined10_lmperturb_cl_multi_neg_synsets_1614_norandomneg_bert_large/Step12000/pytorch_model.bin --train_file piqa.py --shots 64